            ...
            && (name[i] != '\\' || (i < name_len && isPUNCT_A(name[i+1]))))
        {
            ...
            if (special_property || table_index) {
                ...
                if (special_property) {
                    ...
                    table = load_charnames(newSVpvs("placeholder"),
                                           revised_name, name_len,
                                           &error_msg);
                    if (table == NULL) {
                    ...
                    if (handle_names_wildcard(revised_name, name_len,
                                &prop_definition))
                    {
                ...
                prop_values = UNI_prop_value_ptrs[table_index];
                              ...
                              SV ** prop_definition)
{
    ...
    /* And match against the string of all names /gc */
    ...
    { /* Here, matched. */
        ...
        char * this_name_end   = all_names_start + RX_OFFS(subpattern_re)->end;
        char * cp_start;
        char * cp_end;
        UV cp;
        ...
        /* If we didn't find a \n, it means it matched somewhere in the initial
         * '00000' in the string, so isn't a real match */
        if (cp_end == NULL) {
            continue;
        }

        this_name_start = cp_end + 1;   /* The name starts just after the \n */
        cp_end--;                       /* and the code point ends just before
                                           it */
         ...
         * code point is \n\n.  If that isn't the case here, we didn't match
         * the name of a character.  (We could have matched a named sequence,
    ...
    { /* First see if the subpattern matches any of the algorithmic generatable
       ...
       * Unicode standard, version 5.1, section 3.12, and should never change.
       * */
        ...
        for (i = 0; i < SCount; i++) {
            ...
            SV * name = newSVpvs("HANGUL SYLLABLE ");
            sv_catpv(name, JamoL[L - LBase]);
            sv_catpv(name, JamoV[V - VBase]);
            if (T != TBase) sv_catpv(name, JamoT[T - TBase]);
    ...
    for (i = 0; i <= av_top_index((AV *) algorithmic_names); i++) {
         ...
         * in the series, and the low and high code points in them */
        ...
        SV * name = Perl_newSVpvf(aTHX_ "%s-00000", SvPVX(prefix));

        for (j = low; j <= high; j++) { /* For each code point in the series */

            /* Get its name, and see if it matches the subpattern */
            Perl_sv_setpvf(aTHX_ name, "%s-%X", SvPVX(prefix), (unsigned) j);
                         ...
                         0))
    {
        ...
        SV * empty_names_ref = _new_invlist_C_array(uni_prop_ptrs[UNI_C]);
        SV * empty_names = invlist_clone(empty_names_ref, NULL);

        SV * subtract = _new_invlist_C_array(uni_prop_ptrs[UNI_CC]);

        _invlist_subtract(empty_names, subtract, &empty_names);
        SvREFCNT_dec_NN(empty_names_ref);
        SvREFCNT_dec_NN(subtract);

        subtract = _new_invlist_C_array(uni_prop_ptrs[UNI_CF]);
        _invlist_subtract(empty_names, subtract, &empty_names);
        SvREFCNT_dec_NN(subtract);
